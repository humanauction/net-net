...
void StatsAggregator::advanceWindow() {
    // set window to last packet timestamp if available
    if (impl_->last_packet_ts.time_since_epoch().count() != 0) {
        impl_->current.window_start = impl_->last_packet_ts;
    }
    ...
    impl_->current.window_start = std::chrono::system_clock::now();
    impl_->last_packet_ts = {}; // reset last packet timestamp
}
...
void StatsAggregator::ingest(const ParsedPacket& packet) {
    ...
    impl_->last_packet_ts = packet.meta.timestamp;
